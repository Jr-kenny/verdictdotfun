...
class QuizGame(gl.Contract):
    ...
    def create_room(self, room_id: str, category: str, prompt: str):
        ...
        if len(normalized_prompt) < 12:
            raise Exception("Quiz questions must be at least 12 characters.")
    ...
    def submit_entry(self, room_id: str, submission: str):
        ...
        if len(text) < 8:
            raise Exception("Quiz answers must be at least 8 characters.")
    ...
    def resolve_room(self, room_id: str):
        ...
        if room.opponent == ZERO_ADDRESS:
            raise Exception("A quiz room needs two players.")
    ...
    def _build_verdict_prompt(self, room: QuizRoom) -> str:
        return f"""
You are judging a QuizGame room for Verdict Arena.
...
Rules:
- Judge the answers by factual correctness, completeness, and relevance to the question.
- Reward concise accuracy over confident nonsense.
- Penalize unsupported guessing, avoiding the question, and factual mistakes.
...
Player one ({room.owner_name}) answer:
{room.owner_submission}

Player two ({room.opponent_name}) answer:
