...
class PromptDuelGame(gl.Contract):
    ...
    def create_room(self, room_id: str, category: str, owner_profile: Address = ZERO_ADDRESS, stake: u256 = u256(0)):
        ...
        if self.single_room_only and len(self.room_ids) > 0:
            raise gl.vm.UserError("[EXPECTED] This prompt duel room contract is already initialized.")
        ...
        self.rooms[normalized_id] = PromptDuelRoom(
            ...
            target="", owner_submission="", opponent_submission="", status="waiting",
    ...
    def submit_entry(self, room_id: str, submission: str):
        ...
        if room.opponent == ZERO_ADDRESS:
            raise gl.vm.UserError("[EXPECTED] A prompt duel room needs two players.")
        if not room.target or room.status == "ready_to_start":
            raise gl.vm.UserError("[EXPECTED] Start the room before submitting.")
        if room.status != "active":
            raise gl.vm.UserError("[EXPECTED] This prompt duel room is not accepting submissions.")
        if len(text) < 3:
            raise gl.vm.UserError("[EXPECTED] Your prompt must be at least 3 characters.")
        if len(text) > 500:
            raise gl.vm.UserError("[EXPECTED] Prompts must be 500 characters or fewer.")
    ...
    def start_room(self, room_id: str):
        ...
        room.target = self._generate_target(room.id, room.category)
        room.status = "active"
        self.rooms[room.id] = room
...
- Use the room seed "{room_id}" to vary the result.
    ...
    def _open_escrow_if_staked(self, room: PromptDuelRoom):
    ...
    def _finalize_room(self, room: PromptDuelRoom):
        if room.status == "resolved":
            raise gl.vm.UserError("[EXPECTED] Room already has a verdict.")
...
The winner MUST be the player with the higher score.""".strip()
    ...
    def _enter_provisional(self, room: PromptDuelRoom, winner: Address):
    ...
    def _settle_winner(self, room: PromptDuelRoom):
    ...
    def _wager_bonus_xp(self, room: PromptDuelRoom) -> int:
    ...
    def _settle_void(self, room: PromptDuelRoom):
        if self.credit_ledger != ZERO_ADDRESS and int(room.stake) > 0:
            CreditLedgerIface(self.credit_ledger).emit(on="accepted").finalize_void(room.id)

    def _resolved_loser(self, room: PromptDuelRoom) -> Address:
    ...
    def forfeit_room(self, room_id: str):
        ...
        if room.status == "resolved":
            raise gl.vm.UserError("[EXPECTED] Room already has a verdict.")
        if room.opponent == ZERO_ADDRESS:
            raise gl.vm.UserError("[EXPECTED] A prompt duel room needs two players before someone can quit.")
    ...
    def _build_appeal_prompt(self, room: PromptDuelRoom) -> str:
        evidence_note = (
...
{evidence_note}Match claim: {room.target}
...
Return JSON: {{"decision": "upheld" | "overturned", "reasoning": "<one or two sentences>"}}"""
    ...
    def _require_room(self, room_id: str) -> PromptDuelRoom:
        normalized_id = room_id.strip().upper()
    ...
    def _require_profile_owner(self, profile_address: Address) -> Address:
        if self.core_contract == ZERO_ADDRESS:
            if gl.message.sender_address not in self.local_profiles:
                raise gl.vm.UserError("[EXPECTED] Create a local profile before interacting with the prompt duel game.")
    ...
    def _participant_role(self, room: PromptDuelRoom) -> str:
    ...
    def _require_room_owner(self, room: PromptDuelRoom):
        sender = gl.message.sender_address
        if self.core_contract == ZERO_ADDRESS:
            if room.owner != sender:
