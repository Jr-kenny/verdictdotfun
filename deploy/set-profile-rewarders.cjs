...
async function main() {
  ...
  if (!proxyAddress) {
    throw new Error("Set VERDICT_NFT_CONTRACT_ADDRESS or VITE_VERDICT_NFT_CONTRACT_ADDRESS first.");
  }
  if (!operator) {
    throw new Error("Set VERDICT_NFT_OPERATOR.");
  }

  const nonceManagedSigner = new hre.ethers.NonceManager(signer);
  const contract = await hre.ethers.getContractAt("VerdictProfileNft", proxyAddress, nonceManagedSigner);
